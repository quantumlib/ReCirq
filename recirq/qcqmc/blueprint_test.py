...
from qc_afqmc.blueprint import (
    _get_truncated_cliffords,
    BlueprintParamsTrialWf,
    build_blueprint,
    build_blueprint_from_base_circuit,
)
from qc_afqmc.hamiltonian import HamiltonianData, LoadFromFileHamiltonianParams
...
def test_build_blueprint_from_base_circuit():
    trial_wf_params = DummyTrialWfParams(
        name='dummy', hamiltonian_params=LoadFromFileHamiltonianParams("dummy", "dummy", 0, 0)
    )
    blueprint_params = BlueprintParamsTrialWf(
        name='blueprint_test',
        trial_wf_params=trial_wf_params,
        n_cliffords=10,
        qubit_partition=tuple((q,) for q in trial_wf_params.qubits_jordan_wigner_ordered),
        seed=1,
    )
    bp_data = build_blueprint_from_base_circuit(blueprint_params, base_circuit=cirq.Circuit())
...
def test_small_blueprint(
    ...
    blueprint = build_blueprint(blueprint_params, dependencies={trial_wf_params: trial_wf_data})
...
@pytest.mark.slow()
def test_medium(fixture_8_qubit_ham_and_trial_wf: Tuple[HamiltonianData, TrialWavefunctionData]):
...
def test_quaff_respects_seed(seed):
    ...
    truncated_cliffords_1a = [
        [TruncatedCliffordGate.random(len(qubits), rng_1a) for qubits in qubit_partition]
        for _ in range(n_cliffords)
    ]

    truncated_cliffords_1b = [
        [TruncatedCliffordGate.random(len(qubits), rng_1b) for qubits in qubit_partition]
        for _ in range(n_cliffords)
    ]

    truncated_cliffords_2a = [
        [TruncatedCliffordGate.random(len(qubits), rng_2a) for qubits in qubit_partition]
        for _ in range(n_cliffords)
    ]

    truncated_cliffords_2b = [
        [TruncatedCliffordGate.random(len(qubits), rng_2b) for qubits in qubit_partition]
        for _ in range(n_cliffords)
    ]

    resolvers_1a = [get_truncated_cliffords_resolver(gates) for gates in truncated_cliffords_1a]
    resolvers_1b = [get_truncated_cliffords_resolver(gates) for gates in truncated_cliffords_1b]
    resolvers_2a = [get_truncated_cliffords_resolver(gates) for gates in truncated_cliffords_2a]
    resolvers_2b = [get_truncated_cliffords_resolver(gates) for gates in truncated_cliffords_2b]
