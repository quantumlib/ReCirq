...
def test_rotate_wavefunction_explicitly():
    ...
    assert np.allclose(fqe_wf.get_coeff((n_elec, 0)), rotated_fqe_wf.get_coeff((n_elec, 0)))

    one_body_basis_change_mat = np.random.rand(n_orb, n_orb)
    _, one_body_basis_change_mat = np.linalg.eigh(one_body_basis_change_mat + one_body_basis_change_mat.T)
    rotated_fqe_wf = convert_to_ipie.rotate_wavefunction_explicitly(
        fqe_wf, one_body_basis_change_mat, n_qubits
    )
    assert not np.allclose(fqe_wf.get_coeff((n_elec, 0)), rotated_fqe_wf.get_coeff((n_elec, 0)))
...
def test_get_trial_wf_export_data(fixture_8_qubit_ham_and_trial_wf):
    ...
    assert export_data.path == pathlib.Path(trial_wf_data.params.base_path).with_suffix(".h5")
...
def test_get_overlap_export_data(fixture_4_qubit_ham_trial_wf_and_overlap_analysis):
    hamiltonian_data, trial_wf_data, overlap_analysis_data = fixture_4_qubit_ham_trial_wf_and_overlap_analysis
...
def test_get_occa_occb_coeff():
    ...
    data = {(n_elec, 0): np.random.random(size=(n_det_a, n_det_b)) + 1j*np.random.random(size=(n_det_a, n_det_b))}
    ...
    fqe_wf_from_coeff = convert_to_ipie.get_fqe_wf_from_occ_coeff(occ_coeff, n_elec, 0, n_orb)
...
def test_save_wavefunction_for_ipie(fixture_4_qubit_ham_and_trial_wf, ):
