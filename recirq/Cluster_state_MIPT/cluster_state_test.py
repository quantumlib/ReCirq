...
class ClusterStateTest(unittest.TestCase):
    ...
    def test_get_circuit_basis_rotations(self):
        ...
        self.assertTrue(any(isinstance(op.gate, type(cirq.H())) for op in circuit_x.all_operations()))
        ...
        self.assertTrue(any(isinstance(op.gate, type(cirq.Rx(rads=0))) for op in circuit_y.all_operations()))
        ...
        self.assertFalse(any(isinstance(op.gate, (type(cirq.H()), type(cirq.Rx(rads=0)))) for op in circuit_z.all_operations()))
    ...
    def test_get_circuit_ancilla_operations(self):
        ...
        self.assertTrue(any(isinstance(op.gate, type(cirq.H())) and op.qubits[0] in ancilla_qubits
                          for op in circuit.all_operations()))
        
        # Check for CZ gates between physical and ancilla qubits
        self.assertTrue(any(isinstance(op.gate, type(cirq.CZ(qubits_matrix[0][0], qubits_matrix[0][1]).gate)) and
