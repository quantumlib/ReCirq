...
class TestToricCodePlaquettes:
    ...
    def test_for_uniform_parity(self):
        x_value = 0.3
        z_value = -1.0
        plaquettes = tcp.ToricCodePlaquettes.for_uniform_parity(
            self.small_code, x_value, z_value
        )

        expected_x_plaquettes = len(list(self.small_code.x_plaquette_indices()))
        expected_z_plaquettes = len(list(self.small_code.z_plaquette_indices()))
    ...
    def test_expectation_value_x(self):
        # Construct data with -1 on plaquette (0, 0) and +1 on plaquette (0, 1)
        data = pd.DataFrame([0b1101100, 0b0010000, 0b0011101, 0b1000110])
        assert (
            tcp.ToricCodePlaquettes.expectation_value(self.small_code, data, 0, 0, True)
            == -1
        )
        assert (
            tcp.ToricCodePlaquettes.expectation_value(self.small_code, data, 0, 1, True)
            == 1
        )
